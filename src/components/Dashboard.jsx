...
export default function Dashboard() {
  ...
  // 2) Builder: si quedó autorizado, arma el manifiesto de reproducción.
  const handlePlay = async (item) => {
    ...
    try {
      console.log(`🛡️ [Abstract Factory] Autorizando "${item.title}" para usuario #${user.id} (plan ${user.planTipo})...`);
      const autorizacion = await axios.post('http://localhost:5000/api/stream/autorizar', {
        usuarioId: user.id,
        contenidoId,
      });
      console.log('✅ [Abstract Factory] Autorización + licencia recibida:', autorizacion.data);

      console.log(`🧱 [Builder] Construyendo manifiesto de reproducción para "${item.title}"...`);
      const manifiesto = await axios.post('http://localhost:5000/api/stream/manifest', {
        contenidoId,
        tokenDRM: autorizacion.data.licencia.licenciaId,
      });
      console.log('✅ [Builder] Manifiesto listo:', manifiesto.data.manifest);

      setReproduciendo({
        titulo: item.title,
        licencia: autorizacion.data.licencia,
        manifest: manifiesto.data.manifest,
      });
    } catch (err) {
